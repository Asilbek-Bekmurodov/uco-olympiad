...
const Register = ({ onVerifySuccess, onLoginClick }: RegisterProps) => {
  ...
  return (
    <div className="bg-gray-50 min-h-screen flex flex-col md:flex-row">
      {/* LEFT SIDEBAR */}
      <div className="w-full md:w-[35%] p-[1.6rem] flex justify-center md:justify-start">
        <div className="flex flex-col justify-between shadow-neutral-shadow border border-gray-300 h-full bg-gray-100 py-[4rem] px-[2rem] md:px-[3.6rem] rounded-[2.4rem] max-h-[95vh] w-[350px]">
          <div>
            <img
      ...
      <div className="w-full md:w-[65%] flex justify-center items-start md:items-center py-8 md:py-0 px-4 sm:px-6 md:px-0 overflow-x-hidden">
        ...
        {currentStep === 2 && (
          <div className="flex justify-center w-full px-4 sm:px-0 py-8 md:py-0">
            <form
              onSubmit={submitOtp}
              className="flex flex-col items-center border-4 border-gray-200 rounded-4xl p-5 w-full max-w-[400px]"
            >
              <OtpInput value={otp} onChange={setOtp} />
              <button
                type="submit"
                disabled={isVerifying}
                className="w-full h-[5.6rem] rounded-[1.8rem] text-white text-[1.6rem] font-semibold bg-gradient-to-r from-[#6F63FF] to-[#4B2CFF] shadow-[0_20px_40px_-18px_rgba(75,59,255,0.7)] hover:opacity-95 active:scale-[0.98] transition disabled:opacity-60 mt-4"
              >
                {isVerifying ? "Tasdiqlanmoqda..." : "Tasdiqlash"}
              </button>
            </form>
